...
func main() {
	...
	// of waiting: client.Transcribe(path, stt.TranscribeOptions{CallbackURL: "https://..."}, nil)

	// 1. Fire-and-forget: Submit returns a job id immediately, without waiting.
	jobID, err := client.Submit("audio.mp3", stt.TranscribeOptions{})
	if err != nil {
		fmt.Fprintln(os.Stderr, err)
		os.Exit(1)
	}
	...
	if page, lerr := client.ListJobs(10, ""); lerr == nil {
